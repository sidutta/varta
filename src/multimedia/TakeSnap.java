...
public class TakeSnap extends JFrame implements ActionListener, Runnable,
	...
	public void run() {
		...
		webcam.addWebcamListener(TakeSnap.this);
	...
	public static void main(String[] args) {
		SwingUtilities.invokeLater(new TakeSnap());
	}
		...
		if ("capture".equals(e.getActionCommand())) {
			try {
				if (webcam != null) {
					String rand_str = "";
					try {
						rand_str = RandomStringGenerator.generateRandomString(
								10, RandomStringGenerator.Mode.ALPHANUMERIC);
					} catch (Exception e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
					}
					// ImageIO.write(webcam.getImage(), "PNG", new
					// File("/Users/Siddhartha/NetBeansProjects/"+rand_str+".png"));
					ImageIO.write(webcam.getImage(), "PNG", new File(
							"/Users/aditya/Downloads/" + rand_str + ".png"));

					LoginController.client
							.connMessage(
									10,
									LoginController.client.getUsername(),
									LoginController.client.getChatController()
											.getRec(),
									rand_str
											+ ":"
											+ LoginController.client
													.getChatController().ttl
													.toString());

				}
			} catch (IOException ex) {
				Logger.getLogger(TakeSnap.class.getName()).log(Level.SEVERE,
						null, ex);
			}
