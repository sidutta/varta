...
import multimedia.handler.frame.FrameDecoder;
		...
		if (decodeWorker != null) {
			...
			if (msg == null) {
				throw new NullPointerException(
						"you cannot pass into an null to the decode");
			}
			...
			if (iPacket.getByteBuffer().get() != -1) {
				...
				IVideoPicture picture = IVideoPicture.make(
						IPixelFormat.Type.YUV420P, dimension.width,
						dimension.height);
				try {
					...
					while (postion < packageSize) {
						postion += iStreamCoder.decodeVideo(picture, iPacket,
								postion);
						if (postion < 0) {
						...
						if (picture.isComplete()) {
							IConverter converter = ConverterFactory
									.createConverter(type.getDescriptor(),
											picture);
				...
				while (offset < iPacket.getSize()) {
					...
					int bytesDecoded = iAudioStreamCoder.decodeAudio(samples,
							iPacket, offset);
					if (bytesDecoded < 0) {
						throw new RuntimeException(
								"got error decoding audio in stream");
					}
