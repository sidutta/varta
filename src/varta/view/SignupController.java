...
public class SignupController {
		...
		signup_button.setOnAction((event) -> {
			...
			// Sh:Check if password and confirm password match
				if (!passwd.getText().equals(confirmpasswd.getText())) {
					...
					LoginController.client = new Client(MainApp.our_server,
							userid, 5002);
					LoginController.client.setSignupController(this);

					String all_data = name + ":" + password + ":" + userid;
					LoginController.client.connMessage(2,
							LoginController.client.getUsername(), null,
							all_data);
				}
	...
	public void signupValid() {
		...
		Platform.runLater(new Runnable() {
			...
			public void run() {
				try {
					Stage loginStage = (Stage) signup_button.getScene()
							.getWindow();
					Stage chatStage = new Stage();
					chatStage.setTitle("Varta. Welcome "
