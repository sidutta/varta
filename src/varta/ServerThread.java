		...
		try {
			...
			while (true) {
					...
					try
					{
						...
						if(rs.next())
						{
							...
							if (server.addFriend(packet.getSender(), 
									packet.getReceiver(), true)
									== 1)
							{
								//packet.setMessage("Already in friend list");
								server.forwardToReceiver(new Packet(1,"Server",packet.getSender(),
							...
							{
								System.out.println( "Sending " + packet );
								server.forwardToReceiver(new Packet(1,"Server",packet.getReceiver(),
										packet.getMessage() ) );
								server.forwardToReceiver(new Packet(1,"Server",packet.getSender(),
										packet.getMessage() ) );
							}
				...
				else if ((packet.getType() == 8) )// is a getFriends message
				{
					...
					server.forwardToReceiver( new Packet (8,"Server",packet.getSender()
				...
				else if(packet.getType() == 16) //Random conversation.
				{
					...
					{
						server.forwardToReceiver( new Packet (1,receiver,packet.getSender()
								,receiver+"| will talk to you now.") );
						server.forwardToReceiver( new Packet (1,packet.getSender(),receiver
								,packet.getSender()+"| will talk to you now.") );
					}
					else
					{
				...
				{//Client requested for another client's webcam
					if(server.webcamAliveMapping.containsKey(packet.getReceiver()))
					{//User Webcam is available
						server.forwardToReceiver( new Packet(15,"Server",packet.getSender(),server.webcamAliveMapping.get(packet.getReceiver())) );
					}
					else
					{//User Webcam not available
						server.forwardToReceiver( new Packet(15,"Server",packet.getSender(),"Webcam Not Found" ));
                                                server.forwardToReceiver( new Packet(1,"Server",packet.getSender(),"Webcam for "+packet.getReceiver()+" is not available" ));
